...
const APP_VERSION = (() => {
    try { return require('../package.json').version || '2.0.0'; }
    catch { return '2.0.0'; }
})();
...
function createApp() {
    ...
    const staticFiles = express.static(C.PUBLIC_DIR, {
        index: false,
        dotfiles: 'deny',
        etag: true,
    ...
    app.use('/generated', express.static(C.GENERATED_DIR, {
        index: false,
        dotfiles: 'deny',
        immutable: C.IS_PRODUCTION,
        maxAge: C.IS_PRODUCTION ? '7d' : 0
    }));

    const upload = multer({
        storage: multer.memoryStorage(),
        limits: { fileSize: C.MAX_UPLOAD_MB * 1024 * 1024, files: 2, fields: 20 }
    });
    const cardUploadFields = upload.fields([
    ...
    app.post('/api/auth/register', registerLimiter, requireTrustedOrigin, (req, res) => {
        ...
        const account = {
            id: S.randomId(), username, usernameLower: username,
            passwordHash: S.createSecretHash(password), active: C.AUTO_ACTIVATE_ACCOUNTS,
            isAdmin: false, createdAt: new Date().toISOString(), lastLoginAt: null
        };
        ...
        return res.status(201).json({
            ...
            message: account.active ? 'Conta criada. Você já pode entrar.' : 'Conta criada. Aguarde a ativação por um administrador.'
        });
    });
    ...
    app.post('/api/auth/login', loginLimiter, requireTrustedOrigin, (req, res) => {
        ...
        if (!account || !S.verifySecret(password, account.passwordHash)) {
    ...
    app.post('/api/keys', requireTrustedOrigin, requireAuth, (req, res) => {
        ...
        const keys = S.loadApiKeys();
        if (keys.filter(k => k.accountId === req.account.id).length >= C.MAX_KEYS_PER_ACCOUNT) {
            return res.status(409).json({ ok: false, error: `Limite de ${C.MAX_KEYS_PER_ACCOUNT} chaves por conta atingido.` });
        }
        const apiKey = createApiKey();
        const record = makeApiKeyRecord(req.account.id, name, apiKey);
        keys.push(record);
        S.saveApiKeys(keys);
        return res.status(201).json({
            ok: true,
            key: S.publicKeyView(record),
            apiKey,
            message: 'Chave criada. Ela poderá ser copiada novamente pelo painel.'
        });
    });

    app.get('/api/keys/:id/reveal', requireAuth, (req, res) => {
        const key = S.loadApiKeys().find(k => k.id === req.params.id && k.accountId === req.account.id);
        if (!key) return res.status(404).json({ ok: false, error: 'Chave não encontrada.' });
        if (!key.keyCipher) {
            return res.status(409).json({ ok: false, legacy: true, error: 'Esta chave foi criada antes do recurso de recuperação. Substitua-a para poder copiar a chave completa.' });
        }
        const apiKey = S.decryptApiKey(key.keyCipher);
        if (!apiKey || S.hashKey(apiKey) !== key.keyHash) {
            return res.status(500).json({ ok: false, error: 'Não foi possível recuperar esta chave. Substitua-a para gerar uma nova.' });
        }
        res.setHeader('Cache-Control', 'no-store');
        return res.json({ ok: true, apiKey });
    });

    app.post('/api/keys/:id/rotate', requireTrustedOrigin, requireAuth, (req, res) => {
        const keys = S.loadApiKeys();
        const key = keys.find(k => k.id === req.params.id && k.accountId === req.account.id);
        if (!key) return res.status(404).json({ ok: false, error: 'Chave não encontrada.' });
        const apiKey = createApiKey();
        key.keyHash = S.hashKey(apiKey);
        key.keyCipher = S.encryptApiKey(apiKey);
        key.preview = `${apiKey.slice(0, 14)}...${apiKey.slice(-6)}`;
        key.active = true;
        key.lastUsedAt = null;
        key.requestCount = 0;
        ...
        S.saveApiKeys(keys);
        res.setHeader('Cache-Control', 'no-store');
        return res.json({ ok: true, key: S.publicKeyView(key), apiKey, message: 'Chave substituída com sucesso.' });
    });
    ...
    app.post('/api/uploads', requireTrustedOrigin, requireAuth, upload.single('file'), async (req, res, next) => {
        try {
            ...
            if (uploads.filter(u => u.accountId === req.account.id).length >= C.MAX_UPLOADS_PER_ACCOUNT) {
                throw Cards.clientError(`Limite de ${C.MAX_UPLOADS_PER_ACCOUNT} uploads por conta atingido.`, 409);
            }
            ...
            const record = {
                id, accountId: req.account.id, originalName: safeOriginalName(req.file.originalname),
                filename, mime: checked.mime, size: checked.buffer.length,
                width: checked.width, height: checked.height, createdAt: new Date().toISOString()
            };
            uploads.push(record);
    ...
    app.get('/api/uploads', requireAuth, (req, res) => res.json({
        ok: true,
        uploads: S.loadUploads().filter(u => u.accountId === req.account.id).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map(S.publicUploadView)
    }));
    ...
    app.get('/api/generations', requireAuth, (req, res) => res.json({
        ok: true,
        generations: S.loadGenerations().filter(g => g.accountId === req.account.id).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 100).map(S.publicGenerationView)
    }));
    ...
    app.patch('/api/admin/accounts/:id', requireTrustedOrigin, requireAdminAuth, (req, res) => {
        ...
        if ((!active || !isAdmin) && account.isAdmin && account.active && accounts.filter(a => a.isAdmin && a.active).length <= 1) {
    ...
    app.delete('/api/admin/accounts/:id', requireTrustedOrigin, requireAdminAuth, (req, res) => {
        ...
        const uploads = S.loadUploads();
        uploads.filter(u => u.accountId === target.id).forEach(u => S.removeFileIfExists(path.join(C.UPLOADS_DIR, u.filename)));
        S.saveUploads(uploads.filter(u => u.accountId !== target.id));
        const generations = S.loadGenerations();
        generations.filter(g => g.accountId === target.id).forEach(g => S.removeFileIfExists(path.join(C.GENERATED_DIR, g.filename)));
        S.saveGenerations(generations.filter(g => g.accountId !== target.id));
    ...
    app.get('/api/admin/keys', requireAdminAuth, (req, res) => {
        ...
        const keys = S.loadApiKeys().sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map(k => {
            const owner = accountMap.get(k.accountId);
            return { ...S.publicKeyView(k), ownerUsername: owner?.username || 'conta removida', ownerActive: Boolean(owner?.active) };
        });
        return res.json({ ok: true, keys });
    });
    ...
    app.get('/api/routes-info', (req, res) => res.json({
        grupos: [
            { nome: 'Cards', routes: [
                { method: 'GET', path: '/generate-card?avatar=&fundo=&textocima=&textopr=&textobaixo=&apikey=', auth: 'apikey', desc: 'Gera PNG 1080x1080 diretamente pela URL' },
                { method: 'POST', path: '/generate-card', auth: 'apikey', desc: 'Gera card via FormData' },
                { method: 'POST', path: '/painel/gerar', auth: true, desc: 'Gera card pelo painel' }
            ] }
        ]
    }));
    ...
    app.use((err, req, res, next) => {
        ...
        if (err instanceof multer.MulterError) {
            return res.status(400).json({ ok: false, error: err.code === 'LIMIT_FILE_SIZE' ? `A imagem ultrapassa o limite de ${C.MAX_UPLOAD_MB}MB.` : 'Upload inválido.' });
        }
...
function createApiKey() {
    return `skynet_${crypto.randomBytes(32).toString('hex')}`;
}
function makeApiKeyRecord(accountId, name, apiKey) {
    return {
        id: S.randomId(), accountId, name,
        keyHash: S.hashKey(apiKey), keyCipher: S.encryptApiKey(apiKey),
        preview: `${apiKey.slice(0, 14)}...${apiKey.slice(-6)}`,
        active: true, createdAt: new Date().toISOString(), lastUsedAt: null, requestCount: 0
    };
}
...
function setSessionCookie(res, token) {
    const parts = [`skynet_session=${encodeURIComponent(token)}`, 'Path=/', `Max-Age=${C.SESSION_DAYS * 86400}`, 'HttpOnly', 'SameSite=Strict'];
