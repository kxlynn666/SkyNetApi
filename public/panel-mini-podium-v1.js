(() => {
  ...
  function style() {
    ...
    el.textContent = `
      ...
      @media(max-width:700px){.panel-mini-podium-head{align-items:flex-start}.panel-mini-podium-meta .button{display:none}.panel-mini-podium-grid{display:grid;grid-auto-flow:column;grid-auto-columns:148px;grid-template-columns:none;overflow-x:auto;scroll-snap-type:x mandatory;padding:6px 2px 8px;max-width:none;overscroll-behavior-inline:contain}.panel-mini-podium-card{scroll-snap-align:start}.panel-mini-podium-card.place-1{transform:none}.panel-mini-podium{margin-top:13px}.panel-mini-self{font-size:7px}}
      @media(max-width:390px){.panel-mini-podium-grid{grid-auto-columns:140px}.panel-mini-content{padding:8px}.panel-mini-person .cosmetic-avatar{width:39px;height:39px}}
    `;
  ...
  async function install() {
    ...
    if (!stats || document.getElementById('panelMiniPodium')) return false;
    ...
    try {
      const [data,me] = await Promise.all([S.api('/api/profile-v3/leaderboard?limit=50'),S.session()]);
      ...
      const self = document.getElementById('panelMiniSelf');
