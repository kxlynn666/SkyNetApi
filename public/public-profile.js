(() => {
    ...
    async function load() {
        ...
        try {
            ...
            root.innerHTML = `<div class="public-profile">${avatar}<div><div class="eyebrow">Perfil SkyNetApi</div><h1>${S.escapeHtml(profile.displayName)}</h1><div class="public-handle">@${S.escapeHtml(profile.username)}</div>${profile.status ? `<div class="public-status">${S.escapeHtml(profile.status)}</div>` : ''}<div class="public-bio">${S.escapeHtml(profile.bio || 'Sem bio pública.')}</div><div class="public-meta"><div><strong>${Number(profile.friendCount || 0)}</strong><span>amigos</span></div><div><strong>${Number(profile.points || 0)}</strong><span>pontos</span></div><div><strong>${S.escapeHtml(formatDateOnly(profile.createdAt))}</strong><span>membro desde</span></div></div><div style="margin-top:22px"><a class="button primary" href="/painel/amigos">Abrir área social</a></div></div></div>`;
