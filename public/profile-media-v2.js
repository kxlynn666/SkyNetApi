(() => {
  if (window.__SKYNET_PROFILE_MEDIA_V2__) return;
  ...
  style.id = 'profileMediaV2Styles';
  style.textContent = `
    .profile-media-v2-tools{display:grid;gap:8px;margin-top:9px}.profile-media-v2-url{width:100%;font-size:11px!important}.profile-media-v2-row{display:flex;gap:7px;flex-wrap:wrap}.profile-media-v2-row .button{flex:0 1 auto}.profile-media-v2-note{font-size:9px;line-height:1.45;color:var(--text-faint,#74747b)}
    .profile-v3-upload-preview video,.profile-v3-preview-bg video,.profile-v3-person .cosmetic-avatar-inner video{width:100%;height:100%;object-fit:cover;display:block}.profile-v3-preview-bg video{opacity:.58}.profile-media-v2-badge{display:inline-flex;align-items:center;gap:5px;margin-top:6px;padding:4px 7px;border:1px solid #303035;border-radius:999px;font:600 8px 'IBM Plex Mono',monospace;color:#a9a9af;background:#111113}.profile-media-v2-badge::before{content:'';width:5px;height:5px;border-radius:50%;background:#57f287}
  `;
  ...
  function addControls(fileInput, usage, currentMedia) {
    if (!fileInput || fileInput.dataset.profileMediaV2 === '1') return;
    fileInput.dataset.profileMediaV2 = '1';
    ...
    tools.className = 'profile-media-v2-tools';
    tools.innerHTML = `<input class="profile-media-v2-url" type="url" inputmode="url" placeholder="https://... (imagem, GIF ou vídeo)"><div class="profile-media-v2-row"><button class="button primary small" type="button" data-profile-media-save>Usar arquivo / link</button>${currentMedia ? '<button class="button small danger" type="button" data-profile-media-clear>Remover mídia</button>' : ''}</div><div class="profile-media-v2-note">JPG, PNG, WebP, GIF, MP4, WebM ou MOV. GIFs e vídeos ficam sem áudio e são limitados aos primeiros 10 segundos.</div>${currentMedia ? `<span class="profile-media-v2-badge">${currentMedia.kind === 'video' ? 'mídia animada' : 'imagem'} ativa</span>` : ''}`;
  ...
  async function patch() {
    ...
    if (view.banner?.kind === 'video') {
      ...
      if (bg && bg.dataset.profileMediaId !== view.banner.id) {
        bg.dataset.profileMediaId = view.banner.id;
        bg.innerHTML = `<video src="${S.escapeHtml(view.banner.url)}" poster="${S.escapeHtml(view.banner.posterUrl||'')}" autoplay muted loop playsinline preload="metadata"></video>`;
      }
    }
  }
