...
new Function(common);
new Function(publicProfile);
new Function(studioClient);
new Function(statusClient);
...
assert(postboot.includes('/workspace-feature-loader-v1.js?v=product-audit-1'), 'Feature loader novo não está cache-busted.');
assert(featureLoader.includes("path === '/painel/perfil/studio'") && featureLoader.includes('/profile-studio-v1.js?v=1'), 'Feature loader não carrega Profile Studio.');
assert(featureLoader.includes("path === '/painel/status'") && featureLoader.includes('/system-status-v1.js?v=1'), 'Feature loader não carrega status.');
...
assert(workspace.includes('/workspace-postboot-v1.js?v=product-audit-1'), 'Workspace pode reutilizar pós-boot antigo em cache.');

console.log(`Product audit self-test OK (${FIELD_DEFS.flatMap(group => group.fields).length} controles do Profile Studio)`);
